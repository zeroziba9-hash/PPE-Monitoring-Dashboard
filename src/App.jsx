...
export default function App() {
  ...
  const violationCount = alertLogs.filter((log) => ['helmet', 'vest', 'both'].includes(log.type)).length
  const complianceRate = Math.max(0, 100 - Math.round((violationCount / (violationCount + 12)) * 100))
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-6">
      <header className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="text-xs px-3 py-2 rounded-md bg-indigo-600 hover:bg-indigo-500"
          >
            분석 시작
          </button>
        </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <KpiCard title="분석 상태" value={statusText} sub={analysisState === 'analyzing' ? `${progress}%` : '최근 영상 기준'} />
      </section>

      {analysisState !== 'idle' && (
        <section className="mb-4 rounded-xl border border-slate-800 bg-slate-900 p-3">
          ...
          <section className="rounded-xl bg-slate-950 border border-slate-800 p-3">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              ...
              <div className="rounded-lg bg-rose-500/10 border border-rose-500/30 p-2">
                ...
                <div className="text-base font-bold text-rose-200">{cameras.length - onlineCount}</div>
              </div>
            </div>
          </section>
            ...
            <div className="flex gap-2 mb-3">
              <button
                ...
                className={`text-xs px-2 py-1 rounded-md ${
                  activeTab === 'alerts' ? 'bg-indigo-600' : 'bg-slate-800 hover:bg-slate-700'
                }`}
              >
                알람 로그
              </button>
              <button
                ...
                className={`text-xs px-2 py-1 rounded-md ${
                  activeTab === 'history' ? 'bg-indigo-600' : 'bg-slate-800 hover:bg-slate-700'
                }`}
              >
                운영 히스토리
              </button>
...
function KpiCard({ title, value, sub, tone = 'default' }) {
  ...
  return (
    <article className={`rounded-xl border p-3 ${toneStyle}`}>
